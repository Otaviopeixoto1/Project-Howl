...
{
    public static MeshData GenerateTerrainFromSampler(MapGenerator sampler, int meshWidth, int meshHeight, float meshScale)
    {
        ...
        for (int y = 0; y < meshHeight; y++)
        {
            for (int x = 0; x < meshWidth; x++)
            {
                ...
                {
                    meshData.AddTriangle(vertexIndex, vertexIndex + meshWidth, vertexIndex + meshWidth + 1 );
                    meshData.AddTriangle(vertexIndex, vertexIndex + meshWidth + 1, vertexIndex + 1 );
                }
